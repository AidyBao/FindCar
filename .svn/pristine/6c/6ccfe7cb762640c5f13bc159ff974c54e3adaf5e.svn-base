...
class ZXAgencyView: UIView {
    ...
    func setUIStyle() {
        ...
        self.confirmBtn.backgroundColor = UIColor.zx_colorWithHexString("#2b75fd")
    ...
    @IBAction func confirmBtnAction(_ sender: UIButton) {
        ...
        //如果max<min,交换位置
        if self.minText.text!.count > 0, self.maxText.text!.count > 0 {
            let min = Int(self.minText.text!) ?? 0
            let max = Int(self.maxText.text!) ?? 0
            if min > max {
