...
class ZXVerifyCarInfoViewController: ZXUIViewController {
    ...
    var taskId: String?
    var plateImageUrl: String?  //用于匹配的车牌路径
    var ocrNumber: String?      //接口识别的车牌
    var inputNumber: String?    //用户输入的车牌
    ...
    override func viewDidLoad() {
        ...
        if plateImageUrl == nil {
            ZXAlertUtils.showAlert(wihtTitle: nil, message: "车牌图片路径为空", buttonText: nil, action: {
                self.navigationController?.popViewController(animated: true)
            })
            return
        }
