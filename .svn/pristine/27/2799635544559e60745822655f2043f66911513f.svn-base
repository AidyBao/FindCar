...
public class ConfirmViewController: UIViewController, UIScrollViewDelegate {
	...
	public override func viewDidLoad() {
        ...
        let rotationGes = UIRotationGestureRecognizer.init(target: self, action: #selector(rotateImage(rotateRecognizer:)))
    ...
    fileprivate var deltaRotaion: CGFloat = 0.0 //累计 角度
    //fileprivate var deltaRotaion: CGFloat = CGFloat(Double.pi / 2) //累计
    @objc func rotateImage(rotateRecognizer: UIRotationGestureRecognizer) {
        let rotation = rotateRecognizer.rotation
        self.deltaRotaion += rotation//角度 【度】
        if rotateRecognizer.state == .ended {
            self.lastRotation = 0.0
            return
        }
        let currentTransform = self.imageView.transform
        let detalR = 0.0 - (self.lastRotation - rotation)
        print("detalR:\(detalR)")
        let newTransform = currentTransform.rotated(by: detalR)
    ...
    fileprivate func setImageViewImage(_ image: UIImage) {
	...
	internal func confirmPhoto() {
        ...
        let image = image1
        //let image = image1.zx_rotateByDegree(self.deltaRotaion, isExpand: true)
		...
		if let asset = asset {
            ...
            //self.setImageViewImage(newImage)
            //let tempImg = UIImageView.init(image: newImage)
            //tempImg.frame = CGRect(x: 0, y: 0, width: 200, height: 200)
