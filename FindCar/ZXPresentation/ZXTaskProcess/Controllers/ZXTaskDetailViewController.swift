...
class ZXTaskDetailViewController: ZXUIViewController {
    ...
    var fromCarMatch: Bool = false//是否来源拍照匹配（是：验证车辆不再验证车牌）
    var taskModel: ZXTaskModel?
    var taskId: String?
    var zxplateImageUrl: String?    //用于识别的车牌图片
    var ocrNumber: String?          //接口识别的车牌
    var inputPlateNumber: String?   //手动输入的车牌
            ...
            ZXTaskViewModel.detailInfo(taskId: fetchId, completion: { (success, code, taskModel, errorMsg) in
                ...
                if success {
                    ...
                    code.zx_errorCodeParse(loginInvalid: nil, serverError: {
                        if code == ZXAPI_TASK_INVALID {
                            self.navigationController?.popViewController(animated: true)
                            ZXHUD.showFailure(in: ZXRootController.appWindow()!, text: errorMsg, delay: 3)
                        } else {
...
//                #endif
//                return
                if self.fromCarMatch {//来自拍照匹配 不需要再次验证车牌
                    let verifyCarInfoVC = ZXVerifyCarInfoViewController()
                    verifyCarInfoVC.taskId = self.fetchId
                    verifyCarInfoVC.plateImageUrl = self.zxplateImageUrl
                    verifyCarInfoVC.ocrNumber = self.ocrNumber
                    verifyCarInfoVC.inputNumber = self.inputPlateNumber
                    self.navigationController?.pushViewController(verifyCarInfoVC, animated: true)
                } else {//先验证车牌
...
extension ZXTaskDetailViewController: ZXPlateDetectDelegate {
