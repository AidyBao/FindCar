...
class ZXUploadCarInfoViewController: ZXUIViewController {
    ...
    override func zx_rightBarButtonAction(index: Int) {
        ZXExampleShowViewController.showWithImageNames(["shotpic5", "shotpic6", "shotpic7"], ["车架号位置示例", "车头或车尾照示例", "车辆重大问题照示例"], upoun: self) {
...
extension ZXUploadCarInfoViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 3 {
            ...
            cell.setInputText(strRemark, placeholder: "备注信息")
            return cell
        } else {
...
extension ZXUploadCarInfoViewController: ZXImageInputCellDelegate {
    ...
    func zxImageInputCell(cell: UITableViewCell, selectAt index: Int, pointView: UIView?) {
        if let indexPath = self.tblList.indexPath(for: cell) {
            ...
            switch indexPath.section {
            case 0://车架号
                self.arrReviewImages = self.arrVIN
            case 1://车头或车尾
                self.arrReviewImages = self.arrCars
            case 2:///重大问题
                self.arrReviewImages = self.arrProblems
            default:
                break
            }
        ...
        if let indexPath = self.tblList.indexPath(for: cell) {
            ZXAlertUtils.showAlert(wihtTitle: nil, message: "确定删除该张图片?", buttonTexts: ["取消","确定"], action: { (tag) in
                if tag == 1 {
                    ...
                    switch indexPath.section {
                    case 0://车架号
                        path = self.arrVIN[index].absolutePath
                        self.arrVIN.remove(at: index)
                    case 1://车头或车尾
                        path = self.arrCars[index].absolutePath
                        self.arrCars.remove(at: index)
                    case 2://重大问题
                        path = self.arrProblems[index].absolutePath
...
extension ZXUploadCarInfoViewController: PhotoBrowserDelegate {
