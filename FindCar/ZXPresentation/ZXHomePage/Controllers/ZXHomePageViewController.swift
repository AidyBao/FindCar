...
class ZXHomePageViewController: ZXUIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if !onceLoad {
            self.onceLoad = true
            ZXCommonViewModel.checkUpdate({ (update, url) in
                if update {
                    ZXAlertUtils.showAlert(wihtTitle: nil, message: "有新版本更新!", buttonText: "马上更新", action: {
                        ZXCommonUtils.openURL(url)
                    })
                }
            })
            ZXTakePhotoTips.show(at: self)
        }
    }
